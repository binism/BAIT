...
public class Agent  extends AbstractPlayer{
    ...
    private static double MinCost = Double.POSITIVE_INFINITY;
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapseTimer){
        ...
        for(int k = 0; k < actions.size(); k++){ //对当前每一个action进行Astar搜索；
            ...
            if(IsLegalAction(stCopy.copy(), VisitedStateList)&& !tmp.equalPosition(stCopy.copy())){ //判断当前action是否是有效的action；
                AstarBFS(stCopy, StatePipe,StateCostG, VisitedChildStateList, action);
            }
        }
        stCopy =   stateObs.copy();
    ...
    private void AstarBFS(StateObservation stCopy,
            Queue<StateObservation> StatePipe,
            Queue<Integer> StateCostG,
            Queue<StateObservation> VisitedChildStateList,
